...
function publicMessageLayer(atext, callFun) {
	layer.open({
		...
		closeBtn: 1,
		area: ['280px', '147px'],
		content: "<div class='text-center' style='height: 50px;line-height: 67px;'>你确定要" + atext + "吗？</div>",
        offset:[0,0],
		btn: ['确认', '取消'],
		yes: function(index) {
			layer.close(index);
...
function MessageLayer(atext, callFun) {
	layer.open({
		...
		closeBtn: 1,
		area: ['280px', '147px'],
		content: "<div class='text-center' style='height: 50px;line-height: 67px;'>" + atext + "</div>",
		btn: ['确认', '取消'],
		yes: function(index) {
			layer.close(index);
			callFun();
...
function layerTwoConfrim($div, mess, message, callbackFun) {
	let $sureBtn = $div.find("button:first");
	let $calselBtn = $div.find("button:first").next();
	let count = 3;
